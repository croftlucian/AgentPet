...
- **Telegram 文件任务隔离**：Telegram 收到 `document/photo/video/audio/voice/animation/sticker/video_note` 时,先新建 `~/Library/Application Support/com.claude.pet/remote-files/telegram/<bot>/<remoteUserID>/<taskID>/`(根目录复用 `RemoteBotProfileStore.dataDirectory`,与 monitor/profiles 同处 com.claude.pet 树下;taskID 自带 yyyyMMdd-HHmmss、不再按 年/月/日 分桶),固定含 `inbox/`、`work/`、`outbox/`、`meta.json`。下载文件只写当前任务 `inbox/`，文件名会清洗掉路径穿越和隐藏前缀；agent 运行目录切到当前任务根目录,提示里只交代当前任务目录；需要传回 Telegram 的结果必须放进 `outbox/`，执行结束后只扫描当前任务 `outbox/` 的普通文件并用 `sendDocument` 回传,不支持发送任意路径。**任务目录不会永久堆积**:`RemoteFileTask.maybeCleanup` 在启动(后台)与每次新建任务后(6 小时节流)回收历史任务——先按 `Settings.remoteFileRetentionDays`(缺省 14 天,按任务目录树内最新文件 mtime 判龄,避免误删刚续接/刚出结果的任务)删超期,再按 `Settings.remoteFileMaxTotalMB`(缺省 2048MB)从最旧往新删到阈值内,最后清掉空壳目录;两阈值任一置 ≤0 即关闭对应维度(无 UI,需要 `defaults write` 调)。清理传输无关、按 `meta.json` 定位任务,日后接飞书文件可直接复用。**旧版曾落在 home 根的 `~/ClaudePetRemoteFiles`**,启动时 `migrateLegacyBaseIfNeeded` 会把历史任务搬到新位置(新位置不存在则整目录原子搬迁,已存在则逐任务合并、撞名加 uuid 后缀绝不覆盖),搬空后删除旧目录、还 home 清净;`defaultBaseDirectory` 是唯一改址点。**meta.json 只记逻辑元数据(平台/bot/用户/工具/请求/文件清单),不落 root/inbox/work/outbox/file.path 等绝对路径**——它们全可由任务目录推导,落了反成搬目录后唯一会失准的字段;早期还曾按 `年/月/日` 分桶,启动 `migrateFlattenDateBucketsIfNeeded` 把带日期壳的旧任务拍平成 `用户/任务` 并 `stripLegacyPathsFromMeta` 洗掉残留旧路径(幂等)。离线自测：`--telegram-file-task-dryrun`(隔离)、`--telegram-file-cleanup-dryrun`(清理 + 迁移 + 拍平)。
