...
`build.sh` 还会写入 `Info.plist`、登记 `claudepet://` URL 协议，并在 `ClaudePet.app/Contents/Resources/` 下生成 `cc` / `cx` 包装命令：投喂优先走这些包装命令，再回退到系统里的 `claude` / `codex`。默认会覆盖安装并启动 `/Applications/ClaudePet.app`;只想生成项目内 app 时用 `INSTALL_APP= OPEN_AFTER_BUILD=0 ./build.sh`。
...
- **远程 Bot 个性化设置**：`RemoteBotSetup` / `RemoteBotProfileStore` 在 `Sources/RemoteControl.swift`，传输无关,Telegram 与飞书共用。配置落 `~/Library/Application Support/com.claude.pet/remote-profiles.json`，隔离键是 `platform + remoteUserID + botID`；首次使用或旧 bot 未完成设置时,普通消息先被状态机拦截,只接受数字 `1`/`2` 选择逐步设置或一段话快速设置,不支持跳过。`/setup` 重新设置、`/profile` 查看、`/reset` 清空并重新引导。设置完成后,状态机会把 bot 名字、用户称呼、说话风格注入到交给 `AgentRunner` 的 prompt；任务监控仍记录用户原始消息。`RemoteFeatureAnnouncement` 在远程遥控启动/保存配置后检查一次性公告,支持 Telegram 白名单 chat 和飞书 open_id 白名单,按每台电脑 + 每个平台 + 每个 bot + 每个会话 + 公告版本去重,白名单为空不推送。离线自测用 `--bot-setup-dryrun` 和 `--announcement-dryrun`。
- **Telegram 文件任务隔离**：Telegram 收到 `document/photo/video/audio/voice/animation/sticker/video_note` 时,先新建 `~/Library/Application Support/com.claude.pet/remote-files/telegram/<bot>/<remoteUserID>/<yyyy>/<MM>/<dd>/<taskID>/`(根目录复用 `RemoteBotProfileStore.dataDirectory`,与 monitor/profiles 同处 com.claude.pet 树下),固定含 `inbox/`、`work/`、`outbox/`、`meta.json`。下载文件只写当前任务 `inbox/`，文件名会清洗掉路径穿越和隐藏前缀；agent 运行目录切到当前任务根目录,提示里只交代当前任务目录；需要传回 Telegram 的结果必须放进 `outbox/`，执行结束后只扫描当前任务 `outbox/` 的普通文件并用 `sendDocument` 回传,不支持发送任意路径。**任务目录不会永久堆积**:`RemoteFileTask.maybeCleanup` 在启动(后台)与每次新建任务后(6 小时节流)回收历史任务——先按 `Settings.remoteFileRetentionDays`(缺省 14 天,按任务目录树内最新文件 mtime 判龄,避免误删刚续接/刚出结果的任务)删超期,再按 `Settings.remoteFileMaxTotalMB`(缺省 2048MB)从最旧往新删到阈值内,最后清掉空壳目录;两阈值任一置 ≤0 即关闭对应维度(无 UI,需要 `defaults write` 调)。清理传输无关、按 `meta.json` 定位任务,日后接飞书文件可直接复用。**旧版曾落在 home 根的 `~/ClaudePetRemoteFiles`**,启动时 `migrateLegacyBaseIfNeeded` 会把历史任务搬到新位置(新位置不存在则整目录原子搬迁,已存在则逐任务合并、撞名加 uuid 后缀绝不覆盖),搬空后删除旧目录、还 home 清净;`defaultBaseDirectory` 是唯一改址点。离线自测：`--telegram-file-task-dryrun`(隔离)、`--telegram-file-cleanup-dryrun`(清理 + 迁移)。
