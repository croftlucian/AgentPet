...
- **Telegram 文件任务隔离**：Telegram 收到 `document/photo/video/audio/voice/animation/sticker/video_note` 时,先新建 `~/ClaudePetRemoteFiles/telegram/<bot>/<remoteUserID>/<yyyy>/<MM>/<dd>/<taskID>/`，固定含 `inbox/`、`work/`、`outbox/`、`meta.json`。下载文件只写当前任务 `inbox/`，文件名会清洗掉路径穿越和隐藏前缀；agent 运行目录切到当前任务根目录,提示里只交代当前任务目录；需要传回 Telegram 的结果必须放进 `outbox/`，执行结束后只扫描当前任务 `outbox/` 的普通文件并用 `sendDocument` 回传,不支持发送任意路径。离线自测：`--telegram-file-task-dryrun`。
