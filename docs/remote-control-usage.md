...
这份说明面向已经构建并启动 AgentPet 的使用者，讲清楚 Telegram 和飞书远程遥控怎么启用、聊天里每个命令怎么用，以及本地自测命令怎么排查。

远程遥控的执行层在本机跑 cc CLI 或 Codex CLI：`cc` 对应 Claude，`cx` 对应 Codex。Telegram 和飞书只是收发消息；真正干活的是当前 Mac 上的 CLI，所以本机要先能正常运行 `claude` 或 `codex`。
...
Telegram 的 cc 和 Codex(cx) 各是一组 bot token。每组都可以填多枚 token，适合“每个人一个独立 bot”。只想用其中一个工具，也可以只填对应工具的 token。

### 1. 创建 bot

1. 在 Telegram 找 `@BotFather`。
2. 给每个需要 cc 的人创建一个 bot，保存 token，填到「cc bot tokens(可多枚)」。
...
飞书只支持国内版 `open.feishu.cn` 的长连接方式。每个工具需要一个企业自建应用：一个给 cc，一个给 Codex(cx)。
...
在 AgentPet 的远程遥控窗口里填写：

| 字段 | 用途 |
| --- | --- |
| `cc App ID` | Claude 远程遥控应用的 App ID |
| `cc App Secret` | Claude 远程遥控应用的 App Secret |
...
```text
/cd ~/other-project/myself/AgentPet
```

成功回复：

```text
已切到 /Users/你/other-project/myself/AgentPet,并开了新会话。
```
...
- 目录里如果需要访问受系统保护的位置，请先给 AgentPet.app 对应的 macOS 权限。
...
```text
当前目录:/Users/你/other-project/myself/AgentPet
```
...
| 入口 | 本机工具 | 远程执行方式 | 会话续接 |
| --- | --- | --- | --- |
| cc bot / cc 飞书应用 | cc CLI | `claude -p ... --output-format stream-json --verbose --dangerously-skip-permissions` | 使用 cc 返回的 `session_id` 和 `--resume` |
...
真跑一轮 CLI，并在终端打印实时进度。这个命令会调用 cc 或 cx，可能消耗额度。
...
5. 如果需要访问受保护目录，给 AgentPet.app 配好 macOS 权限后重启应用。
